...
class _IssueFilterView:
    """Minimal view-shaped shim so ComplexFilterBackend (built for DRF
    request/view lifecycle) can be reused outside it, from this Celery task.
    Only `filterset_class` is read by `_apply_json_filter`/`_validate_fields`.
    """

    filterset_class = IssueFilterSet


def _apply_rich_filters(queryset, rich_filters: dict | None):
    """Apply an optional rich_filters payload to an issue queryset.

    `filters` is the SAME JSON filter-tree shape the Issues List view's own
    richFilters store already produces (e.g. {"and": [{"priority__in":
    "urgent,high"}]}, or a single bare leaf) — applied here via the same
    ComplexFilterBackend the live IssueListEndpoint's filterset relies on, so
    filter semantics never drift between the view and its export.
    `rich_filters` is trusted here (already validated once in
    ExportIssuesEndpoint.post before this task was queued); re-applying
    through the same validating backend call is defense in depth, not a
    ...
    if filter_tree:
        queryset = ComplexFilterBackend()._apply_json_filter(queryset, filter_tree, _IssueFilterView())

    order_by_param = rich_filters.get("order_by")
    if order_by_param:
        # order_issue_queryset returns (queryset, effective_order_by_param) —
        # only the queryset is needed here.
        queryset, _effective_order_by = order_issue_queryset(queryset, order_by_param)
...
def issue_export_task(
    ...
    rich_filters (dict | None): Optional filter/sort scope mirroring the
        Issues List view's active state (see `_apply_rich_filters`). Applied
        as an additional AND-ed narrowing constraint on top of the queryset
        below — it can never widen the exported set past what the requesting
        user's project membership already allows.
    """
    try:
        exporter_instance = ExporterHistory.objects.get(token=token_id)
        ...
        # Narrow to the Issues List view's active filter/sort state, if any.
        # Applied on top of (never in place of) the role-scoped queryset above.
        workspace_issues = _apply_rich_filters(workspace_issues, rich_filters)
